...
const DashboardPage = ({ messages, input, setInput, isLoading, handleSendMessage, handleKeyPress, messagesEndRef, analytics, navbarHeight = "80px" }) => {
    // State for dropdown only
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const [inputValue, setInputValue] = useState('');
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            {/* Chat Messages Area */}
            <div className="flex-1 overflow-y-auto space-y-4 bg-gray-900 border-2 border-red-600 rounded-t-lg scrollbar-hide p-4">
                {messages.map((message, index) => (
                    <div
                        key={index}
                        className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}
            ...
            {/* Input Area - Combined working input typing + working messaging */}
            <div className="flex items-center p-4 bg-gray-900 border-x-2 border-b-2 border-red-600 rounded-b-lg">
                <textarea
                    value={inputValue}
                    onChange={handleInputChange}
                    onKeyDown={handleLocalKeyPress}
